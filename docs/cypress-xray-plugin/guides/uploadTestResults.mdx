...
<figure>
  <ThemedImage
    alt="The plugin allows you to upload Cypress test results to Xray server or Xray cloud."
    sources={{
      light: useBaseUrl("/img/cypressXrayLight.svg"),
      dark: useBaseUrl("/img/cypressXrayDark.svg"),
    }}
  />
  <figcaption>
    The plugin allows you to upload Cypress test results to Xray server or Xray
    cloud.
  </figcaption>
</figure>
...
The plugin will only upload results for tests you have linked to existing [test issues](targetingExistingIssues.mdx).
The plugin will also create a new test execution issue, unless you tell it to [reuse a specific test execution issue](../configuration/jira.mdx#testexecutionissuekey).

:::info Xray Documentation
...
1. :white_check_mark: The first one tries to find an `<h1>` element with text `Example Domain`.
2. :white_check_mark: The second one asserts that the page contains an `<a>` element with a `href` attribute.
3. :x: The third one tries to find an `<img>` element, which does not exist on the page.
...
A corresponding video can be seen [here](../index.mdx).

<Tabs>
  <TabItem value="tab-1" label="demo.spec.cy.js">
...
![xray default iteration](/img/xrayIterationsDefault.png)
...
describe("the login page", () => {
  for (const name of ["Jane", "John", "Mary"]) {
    it(`CYP-123 logs in ${name}`, async () => {
      // ================================================================= //
      // These three options are all equivalent, so choose your favourite! //
      // ================================================================= //
      cy.task("cypress-xray-plugin:task:iteration:definition", {
...
![xray defined iterations](/img/xrayIterationsTask.png)
...
Since we know that the image test will always fail, we'll [mark it as _skipped_](https://github.com/badeball/cypress-cucumber-preprocessor/blob/master/docs/tags.md) to make use of custom Xray statuses (see [here](../configuration/xray.mdx#status) for more information).
...
![uploaded Cucumber execution results](/img/uploadCucumberResults.png)
...
<Tabs>
  <TabItem value="tab-1" label="scenarios.feature">
...
```
  ...
  <TabItem value="tab-3" label="steps.js">
